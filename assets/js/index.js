...
const generateResultBlock = (title, name, i, additionalClass) => {
    return `
        <div class="result result-${name} main-block"><h2 class="title">Результат ${i} ${title}</h2><div class="main-block main-block-white main-block-${name}-${i}">
...
const generateResultRow = (item) => `<div class="results__item">${item}</div>`
...
window.addEventListener('load', () => {
    ...
    const methodsFlags = document.querySelectorAll('[data-checkboxFor]')
    methodsFlags.forEach((item, index) => {
        item.addEventListener('input', (e) => {
            console.log(item.dataset)
            const section = document.querySelector(`[data-checkboxSection="${item.dataset.checkboxfor}"]`)
            if(!section) return

            const sectionInputs = section.querySelectorAll('input:not([type="checkbox"])')

            if (e.target.checked) {
                // methodsWrapper.classList.add(item.dataset)
                sectionInputs.forEach(input => input.setAttribute('required', 'required'))
                section.classList.remove('hidden')
            } else {
                // methodsWrapper.classList.remove(item.id)
                sectionInputs.forEach(input => input.removeAttribute('required'))
    ...
    btnReset.addEventListener('click', () => {
        ...
        const sections = document.querySelectorAll(`[data-checkboxSection]`)
        const inputs = form.querySelectorAll('input:not([type="checkbox"]):not(.common-required)')
    ...
    functionInput.addEventListener('input', (e) => {
        ...
        for (let i = 0; i < output.length; i++) {
            bordersWrapper.innerHTML += `<div class="border">
                            <div class="border__text">Границы<strong>&nbsp${output[i]}</strong>:</div> <span class="scobka">(</span>
                            <input required type="text" data-autocomplete="-10" placeholder="-10" name="leftBorder${output[i]}" id="leftBorder${output[i]}" class="leftBorder common-required"/>
                            <span class="tz">;</span>
                            <input required type="text" data-autocomplete="10" placeholder="10" name="rightBorder${output[i]}" id="rightBorder${output[i]}" class="rightBorder common-required"/>
    ...
    form.addEventListener('submit', (e) => {
        ...
        if (methodsBoolean[0]) {
            ...
            resultWrapper.insertAdjacentHTML('afterbegin', generateResultBlock('Монте-Карло', monteName, monteI))
            ...
            myWorkerMonte.onmessage = function(e) {
                ...
                if (e.data.length !== (variablesArr.length + 2)) {
                    btnSubmit.setAttribute('disabled', 'disabled')
                    btnClear.setAttribute('disabled', 'disabled')
                    progressBar.style.height = `${100 - e.data}%`
                    return false
                }
                ...
                monteLoader.style.display = 'none'
                e.data.forEach((item) => {
                    output.innerHTML += generateResultRow(item)
                })
        ...
        if (methodsBoolean[1]) {
            ...
            resultWrapper.insertAdjacentHTML('afterbegin', generateResultBlock('Имитация отжига', annealingName, annealingI, `${annealingName}-progress`))
        ...
        if (methodsBoolean[2]) {
            ...
            resultWrapper.insertAdjacentHTML('afterbegin', generateResultBlock('Генетический алг.', gaName, GaI))
        ...
        if (methodsBoolean[3]) {
            ...
            resultWrapper.insertAdjacentHTML('afterbegin', generateResultBlock('Интервальный алг.', intervalName, InI))
